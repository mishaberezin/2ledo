...
import SerpControlls from './SerpControlls';
...
const SwipeDeckWithControllsContainer = ({ themedStyle }) => {
  ...
  const onCardDetailClick = opened => {
    setControlsVisible(!opened);
  };

  return (
    <View style={themedStyle.container}>
      <View style={themedStyle.deckContainer}>
        <Deck
          items={DATA}
      ...
      <View style={themedStyle.controlsContainer}>
        <SerpControlls onPress={handleControlPress} visible={controlsVisible} />
      </View>
    </View>
  );
};

const SwipeDeckWithControlls = withStyles(
  SwipeDeckWithControllsContainer,
  () => ({
    container: {
      display: 'flex',
      flex: 1,
...
export default SwipeDeckWithControlls;
